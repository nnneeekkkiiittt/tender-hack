...
Твоя цель — дать пользователю четкий, полезный и доброжелательный ответ на русском языке на основе приведенного текста документации.
Правила:
1. Если описана процедура (например, регистрация, вход, подача заявки, подписание) — опиши понятный пошаговый порядок действий (1., 2., 3.).
2. Если вопрос о конкретной ошибке или кнопке — укажи точное действие в интерфейсе.
3. Не упоминай номера рисунков, схем и таблиц (например, «(Рисунок 1)»).
4. Напиши [NO_CONTEXT] только в том случае, если приведенный текст вообще никак не касается темы вопроса."""
    ...
    def _prepare_messages(
        ...
        if code_matches:
            ...
            user_content = (
                ...
                f"Дай понятный, подробный и структурированный ответ на русском языке на основе документации:"
            )
