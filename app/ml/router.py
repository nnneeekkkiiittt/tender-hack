...
class IntentRouter:
    ...
    def route(self, query: str, history: Optional[List[Dict[str, str]]] = None) -> RouteDecision:
        ...
        try:
            ...
            return RouteDecision(
                ...
                needs_rag=parsed.get("needs_rag", needs_rag)
            )
    ...
    def _heuristic_fallback(self, query: str) -> RouteDecision:
        ...
        l3_keywords = ["500", "error", "ошибка сервера", "упал", "exception", "traceback", "не работает сайт", "crash", "баг"]
        ...
        out_keywords = ["рецепт", "погода", "анекдот", "стих", "фильм", "пирог", "гороскоп", "песня", "привет как дела"]
        ...
        l2_keywords = ["гост", "регламент", "фз-", "закон", "скзи", "арм", "криптопро", "требован", "положен", "статья", "пункт", "закупк"]
