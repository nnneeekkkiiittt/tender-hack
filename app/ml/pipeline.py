...
class SupportMLPipeline:
    ...
    def _create_l3_result(self, query: str, route: RouteDecision) -> RAGResult:
        ...
        return RAGResult(
            answer=(
                ...
                "Ваше обращение автоматически зарегистрировано как инцидент с высоким приоритетом "
                "и передано дежурной инженерной группе третьей линии поддержки (L3)."
            ),
    ...
    def _create_no_context_result(self, query: str, route: RouteDecision) -> RAGResult:
        ...
        return RAGResult(
            answer=(
                ...
                f"Обращение автоматически перенаправлено оператору линии {route.line.value} "
    ...
    def process_stream_setup(
        ...
        draft_card = self._build_context_card(
            query=query,
            route=route,
            bot_answer="",
            chunks=chunks
