...
class MLSettings:
    ...
    MODEL_NAME: str = field(
        default_factory=lambda: os.getenv("MODEL_NAME", "Qwen/Qwen2.5-7B-Instruct-AWQ")
    )

    # TEI / Сервер эмбеддингов (bge-m3)
    TEI_BASE_URL: str = field(
        default_factory=lambda: os.getenv("TEI_BASE_URL", "http://localhost:8080")
    )
    ...
    COLLECTION_L1: str = field(
        default_factory=lambda: os.getenv("COLLECTION_L1", "kb_l1")
    )
    COLLECTION_L2: str = field(
        default_factory=lambda: os.getenv("COLLECTION_L2", "kb_support")
    )
    ...
    SCORE_THRESHOLD: float = field(
        default_factory=lambda: float(os.getenv("SCORE_THRESHOLD", "0.50"))
    )

    # Параметры генерации
    ROUTER_TEMPERATURE: float = 0.0
    GENERATOR_TEMPERATURE: float = 0.1
    MAX_TOKENS_GENERATOR: int = 1024
