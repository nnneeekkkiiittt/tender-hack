    ...
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      ...
      {!isDemoMode && (
        <p className="mt-4 text-sm text-ink-muted">
          Настройки пока недоступны. Изменения не сохраняются.
        </p>
      )}
      <form onSubmit={handleSave} className="mt-6">
            ...
            <fieldset disabled={!isDemoMode} className="space-y-4">
              ...
              <label className="flex items-center gap-2.5 text-sm text-ink">
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, notifyOnHighPriority: e.target.checked }))
                  }
        ...
        <div className="mt-4 flex items-center gap-3">
          <Button disabled={!isDemoMode} type="submit" leftIcon={<Save className="h-4 w-4" />}>
            Сохранить изменения
          </Button>
        ...
        {isDemoMode && (
          <p className="mt-4 text-xs text-ink-muted">
            DEMO MODE: изменения не отправляются на backend — этот экран подготовлен для подключения
            реального API настроек.
          </p>
        )}
      </form>
    </div>
